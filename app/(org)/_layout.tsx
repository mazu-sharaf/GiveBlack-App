...
export default function OrgLayout() {
  ...
  if (!user || user.type !== "charity") {
    return (
      <View style={{ flex: 1, alignItems: "center", justifyContent: "center", padding: 24, backgroundColor: c.background }}>
        <Text style={{ fontFamily: "Poppins_600SemiBold", fontSize: 18, marginBottom: 12, color: c.text }}>
          Organization access only
        </Text>
        <Text style={{ fontFamily: "Poppins_400Regular", fontSize: 14, textAlign: "center", marginBottom: 16, color: c.textMuted }}>
          Please sign in with an approved charity / organization account.
        </Text>
        <Pressable
          onPress={async () => {
            await logout();
          ...
          style={{ paddingHorizontal: 20, paddingVertical: 10, borderRadius: 20, backgroundColor: c.green }}
        >
          <Text style={{ color: "#fff", fontFamily: "Poppins_600SemiBold" }}>Back to login</Text>
        </Pressable>
      </View>
    );
  }
