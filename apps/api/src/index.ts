...
const start = async () => {
  try {
    ...
    if (!env.EXPO_ACCESS_TOKEN) {
      app.log.warn(
        "[push] EXPO_ACCESS_TOKEN is not set — push notification delivery is DISABLED. " +
        "Create an access token at expo.dev → Account → Access Tokens and add it to your environment."
      );
    }
  } catch (error) {
    app.log.error(error);
