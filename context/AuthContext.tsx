...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  async function persistOAuthAuthPayload(
    data: Record<string, unknown>,
    expected: "donor" | "charity"
  ): Promise<boolean> {
    ...
    if (serverType === "donor" && isAutoGeneratedName(name, email)) {
  ...
  async function loginWithGoogleInternal(): Promise<{ success: boolean; error?: string; errorType?: OAuthLoginErrorType }> {
    setIsGuest(false);
    try {
      ...
      const ok = await persistOAuthAuthPayload(payload, "donor");
  ...
  async function loginWithGoogle(): Promise<{ success: boolean; error?: string; errorType?: OAuthLoginErrorType }> {
    return loginWithGoogleInternal();
  }
